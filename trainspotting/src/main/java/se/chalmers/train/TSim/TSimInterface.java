package se.chalmers.train.TSim;
...
public final class TSimInterface {
	 ...
	 *
	 * Further we maintain a vector of TrainEvents indexed by trainId, where a
	 * non-null entry means a fatal event has occurred.
	 */
	...
	/**
	 * Returns the single instance (singleton pattern). In first call, creates an
	 * instance of this class and starts a thread executing its run() method.
	 *
	 */
		...
		while (true) {
			...
			try {
				...
				if (dInfo instanceof CommandStatus)
					...
					/*
					 * Store the error event to make future commands concerning this train result in
					 * an exception.
					 */
	 ...
	 * @throws CommandException if the supplied id was false (NO_SUCH_TRAIN), if the
	 *                          speed was illegal (ILLEGAL_SPEED) or if the train
	 *                          had crashed.
	 *
	 */
	...
	/**
	 * Sets the direction of the specified switch. Valid directions are SWITCH_LEFT
	 * and SWITCH_RIGHT.
	 *
