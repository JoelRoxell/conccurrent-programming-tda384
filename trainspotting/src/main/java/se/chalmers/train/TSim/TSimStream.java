...
/* package */ class TSimStream {
	...
	/**
	 * Reads from the input stream and returns an object representing the message
	 * from TSim.
	 *
	...
	public TSimInformation read() throws UnparsableInputException {
		...
		/*
		 * What if there is something that would result in a TT_EOL. Shouldn't we handle
		 * this here?
		 */
	...
	/**
	 * Help method that reads a string from the tokenizer and returns it or throws
	 * an exception if the next token was not a string. Upon reading EOF the whole
	 * system exits.
	 *
	...
	/**
	 * Help method that reads an integer from the tokenizer and returns it or throws
	 * an exception if the next token was not an integer. Upon reading EOF the whole
	 * system exits.
	 *
	 * @param sTokenizer the stream tokenizer to read from.
